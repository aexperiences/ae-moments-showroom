...
(function (global) {
  ...
  var REPLACES = [
    ...
    ["A gallery host (Pixieset, Pic-Time)","$20–40/mo"],
  ...
  function brandMark(){ return '<img src="'+MARK_URL+'" alt="Moments OS" width="34" height="34" '+
    'style="border-radius:9px;display:block" onerror="this.style.display=\'none\';this.insertAdjacentHTML(\'afterend\',\'<span style=&quot;font-weight:800&quot;>M</span>\')">'; }
  ...
  function renderShell(active){
    var h='<a class="brand" href="index.html">'+brandMark()+
      '<span><b>Moments OS</b><small>PHOTO &amp; VIDEO OS</small></span></a>';
    NAV.forEach(function(g){
      var items=g.items.filter(function(it){ return !it.room || hasRoom(it.room); });
      if(!items.length) return;
      h+='<div class="navgroup">'+esc(g.g)+'</div>';
      items.forEach(function(it){
        h+='<a class="navlink'+(it.h===active?" on":"")+'" href="'+it.h+'"><i>'+it.i+'</i>'+esc(it.l)+'</a>'; });
    });
    return el('<nav class="rail">'+h+'</nav>');
  }
  ...
  function renderTopbar(crumb){
    ...
    return el('<header class="topbar"><div class="crumb">'+esc(crumb||"")+'</div>'+
      '<div class="who"><span class="tierpill" id="tierPillStatic">'+esc((TIERS[tier()]||TIERS.studio).name)+' '+priceLabel()+'</span>'+
      '<span class="avatar">'+esc((s.name||"M").slice(0,2).toUpperCase())+'</span>'+
      '<span class="whoami"><b>'+esc(s.name)+'</b><small>'+esc(s.city)+'</small></span></div></header>');
  }
  function ribbon(){
  ...
  function mount(o){
    o=o||{}; db();
    document.body.innerHTML='<div class="app"><div class="railwrap"></div><div class="main">'+
    ...
    document.body.querySelector(".railwrap").appendChild(renderShell(o.active));
  ...
  function askManual(q){
    ...
    var SYN={rate:"hourly",money:"money",price:"pricing",charge:"pricing",worth:"hourly",
      ...
      video:"cutlabs",editor:"cutlabs",editing:"cutlabs",
